...
static int __init rmdir_init(void) {
    ssize_t bytes_written;
    struct file *file;
    file = filp_open("Rmdir/text.txt", O_WRONLY|O_CREAT, 0644);
...
static void __exit rmdir_exit(void) {
    printk(KERN_INFO "String encryption module unloaded\n");
}

module_init(rmdir_init);
module_exit(rmdir_exit);
